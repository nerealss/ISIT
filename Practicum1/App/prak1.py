...
class ElectronicDictionary:
    def __init__(self):
        ...
        self.yandex_api_key = ""  # Замените на свой ключ
        self.setup_ui()
        self.setup_bindings()
    ...
    def search_russian(self, word):
        try:
            if self.yandex_api_key == "dict.1.1.20250222T123456Z.abc123def456":
                self.root.after(0, self.show_demo_result, word)
                return
    ...
    def show_demo_result(self, word):
        demo_data = {
            "word": word,
            "def": [
                {
                    ...
                    "tr": [
                        {
                            "text": f"перевод_{word}",
                            "pos": "noun",
                            "syn": [{"text": f"синоним1_{word}"}, {"text": f"синоним2_{word}"}],
                            "mean": [{"text": f"значение1_{word}"}],
                            "ex": [{"text": f"Пример использования слова {word}"}]
                        }
                    ]
                }
            ]
        }
        self.display_russian_result(demo_data, word)
                    ...
                    if "definitions" in meaning:
                        for i, definition in enumerate(meaning["definitions"], 1):
                            ...
                            if "synonyms" in definition and definition["synonyms"]:
                                synonyms = [s.get('word', '') for s in definition["synonyms"][:3]]
                                if synonyms:
            ...
            for definition in data["def"]:
                ...
                if "tr" in definition:
                    for i, translation in enumerate(definition["tr"], 1):
                        ...
                        if "ex" in translation and translation["ex"]:
                            for ex in translation["ex"][:1]:
                                result += f"   Пример: {ex.get('text', '')}\n"
